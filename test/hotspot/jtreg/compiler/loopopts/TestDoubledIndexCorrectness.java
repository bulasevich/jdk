...
public class TestDoubledIndexCorrectness {
    ...
    // the entry value is 'init + seed' with a non-constant seed, while the back edge
    // offset is zero: the phi must stay
    static int foreignVarEntry(int init, int limit) {
        int sum = 0, prev = init + seed;
        for (int i = init; i < limit; i++) { sum = sum * 31 + prev; prev = i + 1; }
        return sum * 31 + prev;
    }
    ...
    public static void main(String[] args) {
        ...
        check("foreignVarEntry", FWD1, TestDoubledIndexCorrectness::foreignVarEntry);
