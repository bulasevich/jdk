...
/**
  1. Handle the most common case - user doesn't explicitly set any of the segment sizes
    ...
    Make sure that Non-Nmethod doesn't exceed CodeCache size.

  2. User sets some value
    2.1 User sets Non-Nmethod segment size:
        Subtract size from CodeCache size and divide the rest in half between Profiled and Non-Profiled.
        Make sure that the Non-Nmethod doesn't exceed the CodeCache size.

    2.2 User sets Non-Profiled or Profiled segment size
        Calculate Non-Nmethod size and subtract from CodeCache size. Subtract the size of the Non-Profiled (Profiled) segment
        from the remainder and then allocate the rest of the memory for the Profiled (Non-Profiled) segment.
        Make sure that neither Non-Profiled (Profiled) nor Non-Nmethod segments or both segments together don't exceed the CodeCache size.

    2.3 User sets sizes of both Non-Profiled and Profiled segments
        Subtract the Non-Profiled and Profiled segment sizes from the CodeCache. Allocate the remainder to the Non-Nmethod.
        Make sure that neither Non-Profiled nor Profiled segments, or both segments together, exceed the CodeCache size.

    2.4 User sets sizes for all segments
        Use these values
        Make sure that neither of them nor their sum exceeds CodeCache size.

  3. We don't need Profiled segment
    Use all space designated for Profiled segment for Non-Profiled

  4. We do not need the Non-Profiled segment
    Use all space designated for Non-Profiled segment for Non-Nmethod (Q???)

  5. Make sure all segments meet minimum size requirements

  6. Make sure sum of all segments is equal to ReservedCodeCacheSize if it's set explicitly, adjust ReservedCodeCache size otherwise

**/

void CodeCache::report_cache_minimal_size_error(const char *codeheap, size_t size, size_t required_size) {
  log_debug(codecache)("CodeCache minimum size fail for %s %ld vs %ld", codeheap, size, required_size);
...
// inside CompilationPolicy::initialize()
  ...
  if (!cache_size_set && (total > cache_size || total != cache_size)) {
    log_info(codecache)("ReservedCodeCache size %ld changed to total segments size NonNMethod %ld NonProfiled %ld Profiled %ld = %ld",
                          cache_size, non_nmethod.size, non_profiled.size, profiled.size, total);

    // Adjust RCCS as necessary because it was not set explicitly
    cache_size = total;
  }

  log_debug(codecache)("Initializing code heaps ReservedCodeCache %ld NonNMethod %ld NonProfiled %ld Profiled %ld",
                          cache_size, non_nmethod.size, non_profiled.size, profiled.size);
