...
CodeBlob* CodeCache::allocate(int size, CodeBlobType code_blob_type, bool handle_alloc_failure, CodeBlobType orig_code_blob_type) {
  ...
  while (true) {
    int alloc_size = ((EmulateFragmentation != 1) && (code_blob_type == CodeBlobType::MethodNonProfiled)) ? size * EmulateFragmentation : size;
    cb = (CodeBlob*)heap->allocate(alloc_size);
    if (cb != nullptr) break;
